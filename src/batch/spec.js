...
describe('batch', () => {
	...
	test('should contain the data from the files if array of paths was given', (done) => {
		const arrayInput = [join(mockPath, 'foo.md'), join(mockPath, 'bar.md')];

		batch(arrayInput, 1, async ({ goods, index }) => {
			if (index === 0) {
				expect(goods).toEqual(markdownJSON(basename(goods.path)));
			}
	...
	test('should have the original shape in the callback', (done) => {
		batch(markdownFiles, 1, async ({ index, files }) => {
			if (index === 0) {
				const expected = [
					join(mockPath, 'bar.md'),
					join(mockPath, 'baz.md'),
					join(mockPath, 'foo.md'),
				];
